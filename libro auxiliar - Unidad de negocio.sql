SELECT a.dstrct::varchar,
			...
			--Unidad de Negocio     
			CASE WHEN a.tipodoc = 'CDIAR' AND a.documento_rel IS NOT NULL THEN 
					  (SELECT sp_uneg_negocio_name((SELECT negasoc FROM con.factura WHERE documento = a.documento_rel)))
			     WHEN a.tipodoc = 'FAC' THEN 
					   (SELECT sp_uneg_negocio_name((SELECT negasoc FROM con.factura WHERE documento = a.numdoc)))
				 WHEN a.tipodoc = 'ICA' THEN
				 	 	CASE WHEN a.documento_rel IS NOT NULL THEN
				 	       (SELECT sp_uneg_negocio_name((SELECT negasoc FROM con.factura WHERE documento = a.documento_rel)))	
				 	    ELSE
				 	       (select negasoc from con.factura where documento = (select documento from con.ingreso_detalle where num_ingreso = b.numdoc and item = 1))	
				 	    END
				 WHEN a.tipodoc = 'ING' THEN
				 	  (SELECT sp_uneg_negocio_name((SELECT negasoc FROM con.factura WHERE documento = a.documento_rel)))	
				 WHEN a.tipodoc = 'NDC' THEN
				      (SELECT sp_uneg_negocio_name((SELECT negasoc FROM con.factura WHERE documento = a.documento_rel)))
				  /*   WHEN a.tipodoc = 'FAC' THEN 
					   (SELECT sp_uneg_negocio_name((SELECT negasoc FROM con.factura WHERE documento = a.numdoc)))
			     WHEN a.tipodoc = 'ING' AND A.documento_rel IS NOT NULL THEN 
					   (SELECT sp_uneg_negocio_name((SELECT negasoc FROM con.factura WHERE documento = a.documento_rel)))				 
				 WHEN a.tipodoc = 'ING' AND A.documento_rel IS NULL THEN 
				 	   (SELECT sp_uneg_negocio_name((select negasoc from con.factura where documento = (select documento from con.ingreso_detalle where num_ingreso = b.numdoc and item = 1))))
				 WHEN (A.tipodoc = 'CDIAR' OR A.tipodoc = 'ICA' OR A.tipodoc = 'NDC') AND a.documento_rel IS NOT NULL  THEN 
				 	  (SELECT sp_uneg_negocio_name((SELECT negasoc FROM con.factura WHERE documento = a.documento_rel)))
				 WHEN a.tipodoc = 'CDIAR' AND A.documento_rel = '' THEN
				 		(select substring(detalle,1,7) AS negocio from con.comprodet WHERE grupo_transaccion = a.grupo_transaccion GROUP BY negocio) */
  ...
  a.cuenta IN ('13050521','13050905','13050902')
  and a.reg_status = ''
  AND a.creation_date::date BETWEEN '2017-12-31' AND '2019-01-06'
  ORDER BY a.cuenta, a.auxiliar, fechadoc, a.tipodoc, a.numdoc;
  
 --select * from con.comprobante LIMIT 1000WHERE creation_date ='';
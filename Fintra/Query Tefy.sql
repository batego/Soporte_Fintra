...
SELECT   tipo_ref1
	...
	,documento,cuenta,fecha_factura,fecha_vencimiento,ndiasf,clasificacion1 as fiducia,
...
((SELECT ref1,ndiasf	
FROM (	
SELECT fra.ref1,CURRENT_DATE-(CASE WHEN tipo_ref2='SV' THEN 	
...
SELECT tipo_ref1,fra.ref1
	...
	cc.sector AS sector
	/*(select department_name from estado  inner join ciudad on(department_code= coddpt)
		where codciu=cc.ciudad) as sector*/
...
SELECT tipo_ref1,ref1
	...
	, '' AS cod_padre
	, '' AS nics_padre
	,valor_factura, valor_saldo, documento,cuenta, fecha_factura, fecha_vencimiento,
...
SELECT fra.ref1,CURRENT_DATE-(CASE WHEN tipo_ref2='SV' THEN 	
		 ...
		 END)::DATE  AS ndiasf
           ...
           WHERE    valor_saldo<>0 and
                    fra.dstrct = 'FINV'       
                    --AND fra.valor_saldo >0            
		    AND	fra.cmc IN ('OV','OP','FM','CM' ,'PM' ,'FP','RF','FU')
                    AND fra.reg_status != 'A' AND tipo_ref2='SV' --and fra.documento='PM38312_16'
                    AND (SUBSTR(fra.documento,1,2) in ('PM','RM') 
				OR NOT EXISTS(SELECT ff.documento FROM con.factura ff WHERE ff.documento=REPLACE(fra.documento,'N','P') and ff.reg_status!='A')
			)
...
SELECT fra.ref1,CURRENT_DATE-(CASE WHEN tipo_ref2='SV' THEN 	
		 ...
		 END)::DATE  AS ndiasf
           ...
           WHERE    valor_saldo<>0 and
                    fra.dstrct = 'FINV'       
                   -- AND fra.valor_saldo >0            
		    AND	fra.cmc IN ('OV','OP','FM','CM' ,'PM' ,'FP','RF','SR','CV','FU')
                    AND fra.reg_status != 'A' AND tipo_ref2='SV' --and fra.documento='PM38312_16'

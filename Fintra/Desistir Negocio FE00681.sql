...
VALUES ('', 'FINV',125984 , 'DES', 'BTERRAZA', NOW(),  'FE00681', 'Cliente desistio, segun lo informado por  Yosiris Suarez ');
...
CREATE TABLE TEM.FE00681_CXP AS				--SELECT * FROM TEM.FE00681_CXP
select * from fin.cxp_doc
--delete from fin.cxp_doc
where documento_Relacionado in ('FE00681')

CREATE TABLE TEM.FE00681_CXP_IT AS
select * from fin.cxp_items_doc
--delete from fin.cxp_items_doc
where documento in ('FE00681')

select * from con.factura --LIMIT 20	SELECT * FROM CON.FACTURA 
--delete from con.factura
where negasoc in ('FE00681')

select * from con.factura_detalle
-- delete from con.factura_detalle
where documento in (select documento from con.factura where negasoc in ('FE00681'))

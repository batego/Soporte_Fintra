...
VALUES ('', 'FINV',130932, 'DES', 'BTERRAZA', NOW(),  'MC15690', 'Cliente desistio, segun lo informado por  HENRRY FERRER ');
...
where documento_Relacionado in ('MC15690') and descripcion != 'CXP A EFECTY EFECTY'
...
CREATE TABLE TEM.MC15690_FAC AS						--SELECT * FROM TEM.MC15690_FAC
select * from con.factura --LIMIT 20	SELECT * FROM CON.FACTURA 
--delete from con.factura
where negasoc in ('MC15690')


CREATE TABLE TEM.MC15690_FAC_DET AS
select * from con.factura_detalle
-- delete from con.factura_detalle
where documento in (select documento from TEM.MC15690_FAC where negasoc in ('MC15690'))

CREATE TABLE TEM.MC15690_EGRE AS
select * from egreso
--delete from egreso
where document_no in ('EG60158')

CREATE TABLE TEM.MC15690_EGREDET AS

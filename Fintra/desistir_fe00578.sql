...
VALUES ('', 'FINV',125622, 'DES', 'BTERRAZA', NOW(),  'FE00578', 'Cliente desistio, segun lo informado por  Tesillo Luisa ');
...
CREATE TABLE TEM.FE00578_CXP AS				--SELECT * FROM TEM.FE00578_CXP
select * from fin.cxp_doc
--delete from fin.cxp_doc
where documento_Relacionado in ('FE00578')

CREATE TABLE TEM.FE00578_CXP_IT AS
select * from fin.cxp_items_doc
--delete from fin.cxp_items_doc
where documento in ('FE00578')

select * from con.factura --LIMIT 20	SELECT * FROM CON.FACTURA 
--delete from con.factura
where negasoc in ('FE00578')

select * from con.factura_detalle
-- delete from con.factura_detalle
where documento in (select documento from con.factura where negasoc in ('FE00578'))

...
VALUES ('', 'FINV',128263 , 'DES', 'BTERRAZA', NOW(),  'FE01325', 'Cliente desistio, segun lo informado por Henrry Ferrer');



/****************************************
Nota: no se encontraron documentos 
contables ni operativos para este negocio
*****************************************/
...
CREATE TABLE tem.cpx_FE01325 as
select * from fin.cxp_doc
--delete from fin.cxp_doc
where documento_Relacionado ilike ('%FE01325%')

CREATE TABLE tem.cpx_items_FE01325 as
select * from fin.cxp_items_doc
--delete from fin.cxp_items_doc
where documento in ('FE01325')

CREATE TABLE tem.fact_FE01325 as
select * from con.factura 
--delete from con.factura
where negasoc in ('FE01325')

CREATE TABLE tem.fact_det_FE01325 as
select * from con.factura_detalle
-- delete from con.factura_detalle
where documento in (select documento from con.factura where negasoc in ('FE01325'))

CREATE TABLE tem.egre_FE01325 as
select * from egreso
--delete from egreso
where document_no in ('EG54902')

CREATE TABLE tem.egredet_FE01325 as
...
CREATE TABLE tem.comprob_FE01325 as
select * from con.comprobante
--delete from con.comprobante
where numdoc in ('FE01325')

CREATE TABLE tem.comprodet_FE01325 as
select * from con.comprodet
--delete from con.comprodet
where numdoc in ('EG54902')
...
---DIFERIDOS (DIFERIDOS)
CREATE TABLE tem.fen_FE01325 as

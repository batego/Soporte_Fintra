...
VALUES ('', 'FINV',125947 , 'DES', 'BTERRAZA', NOW(),  'MC14958', 'Cliente desistio, segun lo informado por Miguel Lengua');
...
CREATE TABLE TEM.MC14958_CXP AS		--select * from  TEM.MC14958_CXP
select * from fin.cxp_doc
--delete from fin.cxp_doc
where documento_Relacionado in ('MC14958')

CREATE TABLE TEM.MC14958_CXP_ITMS AS 
select * from fin.cxp_items_doc
--delete from fin.cxp_items_doc
where documento in ('MC14958')

CREATE TABLE TEM.MC14958_FACT AS
select * from con.factura 
--delete from con.factura
where negasoc in ('MC14958')

CREATE TABLE TEM.MC14958_FACDET AS
select * from con.factura_detalle
-- delete from con.factura_detalle
where documento in (select documento from con.factura where negasoc in ('MC14958'))

CREATE TABLE TEM.MC14958_EGR AS
select * from egreso
--delete from egreso
where document_no in ('EG54902')

CREATE TABLE TEM.MC14958_EGREDET AS
...
CREATE TABLE TEM.MC14958_COMPR AS 
select * from con.comprobante
--delete from con.comprobante
where numdoc in ('MC14958')

CREATE TABLE TEM.MC14958_COMPRODET AS
...
---DIFERIDOS (DIFERIDOS)
CREATE TABLE TEM.MC14958_FENALCO
select * from ing_fenalco
--delete from ing_fenalco
where codneg in ('MC14958')

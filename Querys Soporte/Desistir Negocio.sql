...
--update negocios set update_user = 'MCAMARGO', estado_neg = 'D'
where cod_neg in  ('MC12094')
...
VALUES ('', 'FINV',108278 , 'DES', 'MCAMARGO', NOW(),  'MC12094', 'Cliente desistio, segun lo informado por Miguel Lengua');
...
where documento_Relacionado in ('MC12094')

select * from fin.cxp_items_doc
--delete from fin.cxp_items_doc
where documento in ('MC07043')

select * from con.factura LIMIT 20
--delete from con.factura
where negasoc in ('MC12094')

select * from con.factura_detalle
-- delete from con.factura_detalle
where documento in (select documento from con.factura where negasoc in ('MC12105'))
...
-- ACTUALIZAR ESTADO DE LA FIANZA DEL NEGOCIO
select * from administrativo.historico_deducciones_fianza where negocio ='' 
estado_proceso = 'A'
reg_status = 'A'

...
SELECT 
	 ...
	 count(0) as docPagos
            ...
            WHERE
                 ...
                 WHERE sp.identificacion='106795460' and sa.id_convenio in ('10','11','12','13') and sa.estado_sol in ('T') and sp.tipo='S'
...
where identificacion='106795460' and numero_solicitud in (89805,107774)
...
SELECT *--case when  ( now() :: date  > min(fecha_vencimiento) :: date  ) then 'N' else 'S' end as aldia
    ...
    ------MODIFICADO
SELECT case when
     ((nro_docs <= 14) and (docpagos >= nro_docs::numeric*.70)) or (((nro_docs::numeric-docpagos) <= 4) and (nro_docs >= 15)) then 'cumple' else 'no' end
     from
    (SELECT 

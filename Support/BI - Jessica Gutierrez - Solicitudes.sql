SELECT
            ...
            FROM (
                select
                    ...
                    LEFT JOIN (select cod_neg, max(fecha) as mifecha ,causal from negocios_trazabilidad where concepto='RECHAZAD' group by cod_neg,causal) traza on (neg.cod_neg = traza.cod_neg)
                    LEFT JOIN tablagen tu ON (tu.table_type='CAUSACONCP' AND referencia = 'RECHAZAD' and traza.causal=tu.table_code)
                    LEFT JOIN (SELECT trazap.numero_solicitud,estado, usuario, fecha ,comentarios,trazap.causal  
                    			FROM apicredit.pre_solicitudes_trazabilidad as trazap
                    ...
                    pre.etapa,tu.dato,sp.celular,pre.email,pre.telefono,pre.tipo_cliente,pre.qb,pre.qm,pre.qa,pre.porc_endeudamiento,pre.rechazo_operaciones,nt.estado,nt.causal
                )temporal WHERE creation_date::date BETWEEN '2018-01-01'::date AND now()::date ;
...
where numdoc in  (select document_no from  egresodet where document_no = 'EG63968')

CREATE TABLE tem.MC16440_egr_comprob AS ;			
select * from con.comprobante 
--delete from con.comprobante 
where numdoc in (select document_no from egreso where document_no = 'EG63968')

CREATE TABLE tem.MC16440_egrdt AS 	;		
select * from egresodet
--delete from egresodet
where document_no in ('EG63968');		

...
where documento_Relacionado in ('MC17151');
...
where numdoc in  (select document_no from  egresodet where document_no = 'EG63970')

CREATE TABLE tem.MC16440_egr_comprob AS ;			
select * from con.comprobante 
--delete from con.comprobante 
where numdoc in (select document_no from egreso where document_no = 'EG63970')

CREATE TABLE tem.MC16440_egrdt AS 	;		
select * from egresodet
--delete from egresodet
where document_no in ('EG63970');		
...
WHERE negasoc ='MC17151';

SELECT * FROM con.factura_detalle 
--delete from con.factura_detalle
WHERE documento IN (SELECT documento FROM con.factura WHERE negasoc ='MC17151');
...
select * from con.comprobante 
--delete from con.comprobante 
where numdoc in (select documento from con.factura where negasoc = 'MC17151');

...
CREATE TABLE tem.MC18701 AS 					
SELECT * FROM fin.cxp_doc WHERE documento_relacionado = 'MC18701';

CREATE TABLE tem.MC18701_det AS 
SELECT * FROM fin.cxp_items_doc WHERE documento IN (SELECT documento FROM fin.cxp_doc WHERE documento_relacionado = 'MC18701');


--restablecer Saldo Cxp
SELECT vlr_neto,* FROM fin.cxp_doc WHERE documento_relacionado = 'MC18701'; -- rest saldo
SELECT vlr_neto,* FROM fin.cxp_doc WHERE documento_relacionado = 'MP15968';

SELECT * FROM fin.cxp_items_doc WHERE documento = 'CXP000006971';

-- Egreso
SELECT * FROM egreso WHERE document_no IN ('EG67921');
--delete FROM egreso WHERE document_no IN ('EG67921');

SELECT documento, document_no, vlr,* 
FROM egresodet WHERE document_no IN ('EG67921');
--delete FROM egresodet WHERE document_no IN ('EG67921');

SELECT * FROM con.comprobante WHERE numdoc IN ('EG67921');
--delete FROM con.comprobante WHERE numdoc IN ('EG67921');

SELECT * FROM con.comprodet WHERE numdoc IN ('EG67921');
--delete FROM con.comprodet WHERE numdoc IN ('EG67921');

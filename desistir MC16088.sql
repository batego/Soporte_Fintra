...
VALUES ('', 'FINV',133363 , 'DES', 'BTERRAZA', NOW(),  'MC16088', 'Cliente desistio, segun lo informado por  HFERRER ');

-- select * from negocios where cod_neg = 'MC16088'
...
CREATE TABLE TEM.MC16088_CXP AS				--SELECT * FROM TEM.MC16088_CXP 
select * from fin.cxp_doc
--delete from fin.cxp_doc
where documento_Relacionado in ('MC16088') AND documento !='CXP000005329';

CREATE TABLE TEM.MC16088_CXP_IT AS			
select * from fin.cxp_items_doc
--delete from fin.cxp_items_doc
where documento in (select documento from fin.cxp_doc where documento_Relacionado in ('MC16088')) AND documento !='CXP000005329';

CREATE TABLE TEM.MC16088_FAC_IT AS
select * from con.factura --LIMIT 20	SELECT * FROM CON.FACTURA 
--delete from con.factura
where negasoc in ('MC16088');

CREATE TABLE TEM.MC16088_FACDET_IT AS
select * from con.factura_detalle
-- delete from con.factura_detalle
where documento in (select documento from con.factura where negasoc in ('MC16088'));

CREATE TABLE TEM.MC16088_EGR_EG62508_IT AS
select * from egreso
--delete from egreso
where document_no in ('EG62695');

CREATE TABLE TEM.MC16088_EGRDET_EG62508_IT AS
...
CREATE TABLE TEM.MC16088_COMPRO_IT AS
select * from con.comprobante
--delete from con.comprobante
where numdoc in ('MC16088');

CREATE TABLE TEM.MC16088_COMPRODET_IT AS
...
---DIFERIDOS (DIFERIDOS)
CREATE TABLE TEM.MC16088_diferidos AS			--select * from TEM.MC16088_diferidos
select * from ing_fenalco
--delete from ing_fenalco
where codneg in ('MC16088');

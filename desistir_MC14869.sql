...
VALUES ('', 'FINV',124474, 'DES', 'BTERRAZA', NOW(),  'MC14869', 'Cliente desistio, segun lo informado por  HENRRY FERRER ');
...
CREATE TABLE TEM.MC14869_CXP AS				--SELECT * FROM TEM.MC14869_CXP
select * from fin.cxp_doc
--delete from fin.cxp_doc
where documento_Relacionado in ('MC14869') and descripcion != 'CXP A EFECTY EFECTY'


CREATE TABLE TEM.MC14869_CXP_IT AS
select * from fin.cxp_items_doc
--delete from fin.cxp_items_doc
where documento in ('MC14869')

CREATE TABLE TEM.MC14869_FAC AS						--SELECT * FROM TEM.MC14869_FAC
select * from con.factura --LIMIT 20	SELECT * FROM CON.FACTURA 
--delete from con.factura
where negasoc in ('MC14869')


CREATE TABLE TEM.MC14869_FAC_DET AS
select * from con.factura_detalle
-- delete from con.factura_detalle
where documento in (select documento from TEM.MC14869_FAC where negasoc in ('MC14869'))

CREATE TABLE TEM.MC14869_EGRE AS
select * from egreso
--delete from egreso
where document_no in ('EG60158')

CREATE TABLE TEM.MC14869_EGREDET AS
...
CREATE TABLE TEM.MC14869_COMPR AS
select * from con.comprobante
--delete from con.comprobante
where numdoc in ('MC14869')


CREATE TABLE TEM.MC14869_COMPRODET AS

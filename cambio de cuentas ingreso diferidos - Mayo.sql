--create table tem.if_mayo2018 as
select * from con.comprodet 
--update con.comprodet set cuenta = '16252141'
where numdoc in (select cod from ing_fenalco where codneg in (select negocio from administrativo.control_endosofiducia group by negocio) and periodo = '201805') 
and valor_credito != 0 and tipodoc = 'IF' and cuenta != '16252141' 

select * from tem.if_mayo2018
							

--select * from tem.if_ABRIL2018
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--select * from tem.if_feb2018_1

--create table tem.if_mayo2018_1 as
select * from con.comprodet 
--update con.comprodet set cuenta = '16252141'
where numdoc in (
	select cod from ing_fenalco where codneg in (
		select lopez.relacionado from (
			...
			where fecha_vencimiento::date between '2018-05-01'::date and '2018-05-31'::date
			and cmc = 'CL'
		) lopez
	)	
)
and periodo = '201805' and valor_credito != 0 and cuenta != '16252141' and tipodoc = 'IF'



select count(*) from  tem.if_mayo2018_1
